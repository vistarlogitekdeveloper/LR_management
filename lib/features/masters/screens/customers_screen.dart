...
class CustomersScreen extends ConsumerWidget {
  ...
  static List<FormFieldSpec> _fields(Customer? c) => [
    FormFieldSpec(
      name: 'name',
      label: 'Customer Name',
      required: true,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final canEdit = user?.canManageCustomers ?? false;

    return MasterPage(
      title: 'Customers',
      subtitle: '${customers.length} parties · auto-fill enabled',
      ...
      onDelete: canEdit
          ? (id) async {
              ...
              if (!ok) return;
              try {
                await ref.read(customersProvider.notifier).remove(id);
              } catch (e) {
